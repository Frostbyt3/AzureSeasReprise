...
namespace Redux.Game_Server
{
    ...
    public unsafe sealed class GameServer
    {
        ...
        public GameServer(string name, int port)
        {
            ...
            //Begin Packet Thread
            new Threading.PacketThread().CreateThread();

            //Begin World Thread
            new Threading.WorldThread().CreateThread();

            //Begin Combat Thread
            new Threading.CombatThread().CreateThread();

            //Start monster thread
            new Threading.MonsterThread().CreateThread();
        ...
        public static void Process_GeneralActionPacket(Player client, GeneralActionPacket packet)
        {
            switch (packet.Action)
            {
                    ...
                    try
                    {
                        ...
                        foreach (var portal in Common.MapService.MapData[mapID].Portals)
                        {
                            if (Space.Calculations.GetDistance(client.X, client.Y, portal.X, portal.Y) < 3)
                            {
                                ...
                                handled = true;
                                Console.WriteLine("Located portal UID {0} ID {1} for map {2}", endPortal.UID, endPortal.MapID, endPortal.MapID);
                                break;
                            }
                        }
                        if (!handled)
                ...
                case DataAction.CompleteLogin:
                    {
                        var data = new UpdatePacket(client.UID);
                        if (client.Character.LuckyTimeExpires > DateTime.Now)
                            data.AddUpdate(UpdateType.LuckyTime, Common.MsFromNow(client.Character.LuckyTimeExpires));
                        if (client.Character.HeavenBlessExpires > DateTime.Now)
                        {
                            data.AddUpdate(UpdateType.HeavenBlessing, Common.SecondsFromNow(client.Character.HeavenBlessExpires));

                            data.AddUpdate(Enum.UpdateType.SizeAdd, 2);
                            if (client.MapID != 601)
                                data.AddUpdate(Enum.UpdateType.OnlineTraining, 0);
                            else
                            {
                                data.AddUpdate(UpdateType.OnlineTraining, 0);
                                data.AddUpdate(UpdateType.OnlineTraining, 1);
                            }
                        }
                        if (client.Character.DoubleExpExpires > DateTime.Now)
                            data.AddUpdate(UpdateType.DoubleExpTime, Common.SecondsFromNow(client.Character.DoubleExpExpires));
                        client.Send(packet);
                        client.Send(data);
                        break;
                    }
                ...
                case DataAction.Mine:
                    if (Common.Clock > client.NextMine && client.Map.MapInfo.Type.HasFlag(MapTypeFlags.MineEnable))
                    {
                        if (client.MineDrops != null && client.MineDrops.MapId != client.MapID)
                            client.MineDrops = new Structures.MiningDrops(client.MapID);
                        else if (client.MineDrops == null)
                            client.MineDrops = new Structures.MiningDrops(client.MapID);
