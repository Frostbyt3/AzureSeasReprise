...
namespace Redux.Space
{
    ...
    public class Map
    {
        ...
        public Map(uint _dynamicID, uint _id)
        {
            ...
            var dSOBs = ServerDatabase.Context.SOB.GetSOBByMap((ushort)_dynamicID);
            if (dSOBs.Count > 0)
                foreach(var sob in dSOBs)
                {
                    SOB newsob = new SOB();
                    newsob.Name = sob.Name;
                    newsob.Mesh = (ushort)sob.Mesh;
                    newsob.Flag = (ushort)sob.Flag;
                    newsob.MaximumLife = (uint)sob.MaxHp;
                    newsob.Life = (ushort)sob.MaxHp;
                    newsob.X = sob.X;
                    newsob.Y = sob.Y;
                    newsob.Life = (uint)sob.MaxHp;
                    newsob.UID = sob.UID;
                    Insert(newsob);
        ...
        public bool IsNoWoundEnabled { get { return !MapInfo.Type.HasFlag(Enum.MapTypeFlags.NeverWound); } }
        public bool IsFreePK { get { return MapInfo.Type.HasFlag(Enum.MapTypeFlags.FreePk); } }
